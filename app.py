...
def get_server_pubkey():
    ...
    if not os.path.exists(key_file):
        ...
        print("Run: wg genkey | tee /etc/wireguard/server_private.key | wg pubkey > /etc/wireguard/server_public.key")
        sys.exit(1)
...
"""
    ...
    os.system(f"wg addconf wg0 <(wg-quick strip wg0) 2>/dev/null || wg set wg0 peer {public_key} preshared-key <(echo '{preshared_key}') allowed-ips {peer_ip}/32")
...
def remove_peer(name):
    ...
    # Get public key from peer file
    pubkey = None
    with open(peer_file) as f:
        private_key_line = [l for l in f if "PrivateKey" in l]
    if private_key_line:
        privkey = private_key_line[0].split("=", 1)[1].strip()
    ...
    if os.path.exists(WG_CONF):
        ...
        for line in lines:
            if f"# Peer: {name}" in line:
                skip = True
            elif skip and line.startswith("# Peer:"):
                skip = False
...
def main():
    if os.geteuid() != 0:
        print("❌ Run as root: sudo python3 wg-add-peer.py")
        sys.exit(1)
